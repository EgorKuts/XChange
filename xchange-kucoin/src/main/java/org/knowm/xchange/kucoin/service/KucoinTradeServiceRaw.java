...
public class KucoinTradeServiceRaw extends KucoinBaseService {
     ...
     * Lists all active orders for a currency pair.
     */
    public KucoinResponse<KucoinActiveOrdersInKv> getKucoinOpenOrdersInKv(
            CurrencyPair currencyPair, OrderType orderType) throws IOException {

        try {
            KucoinResponse<KucoinActiveOrdersInKv> kucoinResponse = kucoin.getAllActiveOrdersInKv(
                    apiKey,
                    exchange.getNonceFactory(),
                    signatureCreator,
            ...
            return checkSuccess(kucoinResponse);
        } catch (KucoinException e) {
            throw new ExchangeException(e.getMessage());
        }
    }
